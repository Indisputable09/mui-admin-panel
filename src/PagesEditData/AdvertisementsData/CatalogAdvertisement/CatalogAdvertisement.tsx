...
import { ICatalogAdvertisement } from '../../../types/advertisementTypes';
...
interface ICatalogAdvertisementProps {
  initialLink: string;
  pageName: string;
...
const CatalogAdvertisement: React.FC<ICatalogAdvertisementProps> = ({
  initialLink,
  pageName,
  parentPageName,
}) => {
  ...
  const [initialData, setInitialData] = React.useState<ICatalogAdvertisement>({
    image: null,
    published: false,
    discount: null,
    url: '',
  ...
  const [fieldsValues, setFieldsValues] =
    React.useState<ICatalogAdvertisement>(initialData);
...
export default CatalogAdvertisement;
