...
import {
  ...
  handleAddVacancy,
  handleDeleteVacancy,
  handleSendVacancyData,
} from '../../services/vacanciesAPI';
import Loader from '../../components/Loader';

interface IVacanciesDataProps {
  initialLink: string;
  pageName: string;
...
const VacanciesData: React.FC<IVacanciesDataProps> = ({
  initialLink,
  pageName,
  parentPageName,
}) => {
  ...
  const [initialData, setInitialData] = React.useState<IVacancy>({
    name: initialValueWithLanguages,
    description: initialValueWithLanguages,
  });
  const [fieldsValues, setFieldsValues] = React.useState<IVacancy>(initialData);
  const [chosenVacancyName, setChosenVacancyName] = React.useState<string>('');
  ...
  React.useEffect(() => {
    if (id) {
      const fetchData = async () => {
        try {
          setStatus(pending);
          const vacancyById = await fetchVacancyById(id as string);
          setFieldsValues(vacancyById);
          setInitialData(vacancyById);
          setStatus(resolved);
        } catch (error) {
          setStatus(rejected);
        }
  ...
  React.useEffect(() => {
    if (fieldsValues) {
      ...
      if (ukName) {
        setChosenVacancyName(ukName!.value);
      }
    }
  }, [fieldsValues]);
      ...
      {status !== pending && status !== rejected && (
        <>
          <PagesDataCommon
            ...
            linksData={{
              link: initialLink,
              name: chosenVacancyName ? chosenVacancyName : null,
              pageName,
              parentPageName,
            }}
            visibilityIcon
          ...
          {openDeleteModal && (
            <Modal
              ...
              type={'delete'}
              link={initialLink}
              handleDeleteData={() => handleDeleteVacancy(id as string)}
            />
          )}
          {openSaveModal && (
            <Modal
              ...
              handleEditData={() =>
                handleSendVacancyData(id as string, fieldsValues)
              }
              handlePostData={() => handleAddVacancy(fieldsValues)}
            />
          )}
        </>
      )}
